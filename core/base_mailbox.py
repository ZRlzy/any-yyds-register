...
DEFAULT_YYDS_MAIL_API_URL = "https://maliapi.215.im"
...
class YYDSMailMailbox(BaseMailbox):
    """YYDS Mail (vip.215.im) — 第三方邮箱 API 服务，支持临时邮箱、验证码原生提取和长轮询。"""
    ...
    def get_email(self) -> MailboxAccount:
        ...
        result = self._api_post("/v1/accounts", body)
        address = result.get("address", "")
        if not address:
            raise RuntimeError("YYDS Mail 创建邮箱失败: 未返回 address")
        token = result.get("token", "")
        print(f"[YYDS Mail] 创建邮箱: {address}")
        return MailboxAccount(
            email=address,
            account_id=result.get("id", ""),
            extra={
                "yyds_mail_token": token,
                "yyds_mail_address": address,
                "provider_resource": {
                    "provider_type": "mailbox",
    ...
    def get_current_ids(self, account: MailboxAccount) -> set:
        address = str((account.extra or {}).get("yyds_mail_address") or account.email)
        try:
            data = self._api_get("/v1/messages", params={"address": address, "limit": 50})
    ...
    def wait_for_link(
        ...
        while time.time() - start < timeout:
            try:
                ...
                link = _extract_verification_link(text, keyword)
                if link:
                    print(f"[YYDS Mail] 获取验证链接: {link}")
                    return link
            except Exception:
                pass
            time.sleep(1)

        raise TimeoutError(f"YYDS Mail 等待验证链接超时 ({timeout}s)")
